...
def namelist_write(site, site_id, lat, lon):
        with open(f'pyVIP_site.txt', 'w') as f:
                ...
                f.write('aeri_path                 = /data/RadianceSim_Output/AERI/IRS_{}_{}_20230419_210000.nc     # Path to AERI ch1 radiance files (default "None")\n'.format(site_id,site))
                ...
                f.write('output_rootname           = {}_{}_AERI_MWR      # String with the rootname of the output file (default None)\n'.format(site_id,site))
                f.write('output_path               = /data/TROPoeSim_Output/AERI_MWR     # Path where the output file will be placed (default None)\n')
...
while i < len(meso_lats):
        if meso_datd[i] >= 20990000:
                ...
                my_file = Path(f"/data/TROPoeSim_Output/AERI_MWR//{meso_ids[i]}_{site_name}_AERI_MWR.20230419.210000.cdf")
                if my_file.is_file():
                        i+=1
                else:
